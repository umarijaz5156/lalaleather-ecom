...
<x-modals.custom-alpine-modal :id="$id" :maxWidth="$maxWidth" {{ $attributes }}>
    <div class="relative w-full h-full md:h-auto ">
        <!-- Modal content -->
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700 modal-main-dev">
            <!-- Modal header -->
            <div class="flex items-start justify-between p-4">
                @isset($headerTitle)
                    <h3 class="text-xl font-semibold text-[#374C98] dark:text-white modal-title">
                        {{ $headerTitle }}
                    </h3>
                @endisset
                <button wire:click="$toggle('{{ $attributes['wire:model'] }}')" wire:loading.attr="disabled" type="button"
