@props(['category','manufacturer' => false])
<div class="bg-[#752D00] rounded-md p-4 space-y-3">
    <h1 class="text-primary uppercase text-xl font-bold text-center">
        {{ ___(\Illuminate\Support\Str::limit($category->title, 14)) }}
    </h1>
    <a type="button" 
    href="{{ ($manufacturer) ? route('manufacturer-catergory-page', ['slug' => $category->slug_store])  : route('category', ['slug_store' => $category->slug_store]) }}">
    <img class="w-full h-[260px] object-cover rounded-md" src="{{ asset('storage/' . $category->banner_path) }}"
        alt="" /></a>
    <a type="button" 
    href="{{ ($manufacturer) ? route('manufacturer-catergory-page', ['slug' => $category->slug_manufacturer])  : route('category', ['slug_store' => $category->slug_store]) }}"
    class="align-middle select-none bg-[#EABE8B] text-darkBrown font-semibold text-center transition-all disabled:opacity-50 disabled:shadow-none disabled:pointer-events-none py-3 px-4 rounded-lg shadow-md shadow-gray-900/10 hover:shadow-lg hover:shadow-gray-900/20 focus:opacity-[0.85] focus:shadow-none active:opacity-[0.85] active:shadow-none flex justify-center items-center gap-3 uppercase w-full"
